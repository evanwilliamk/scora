...
const page = (title: string, content: string) => `<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{background:#000;color:#fff;font-family:system-ui;margin:0;padding:40px;display:flex;align-items:center;justify-content:center;min-height:100vh}.container{max-width:600px;text-align:center}.logo{font-size:100px;margin:0;margin-bottom:20px}.title{font-size:48px;margin:0 0 20px;font-weight:700}.tagline{color:#999;font-size:18px;margin:0 0 50px}ul{list-style:none;margin:40px 0;padding:0;text-align:left;display:inline-block}li{color:#ccc;font-size:16px;padding:12px 0}li:before{content:"•";margin-right:12px;color:#666}.footer{margin-top:60px;padding-top:40px;border-top:1px solid #222;font-size:13px;color:#666}a{color:#fff;text-decoration:none}a:hover{color:#999}</style></head><body><div class="container">${content}</div></body></html>`;

fastify.get('/', async (request, reply) => {
  return reply.type('text/html').send(page('SCORA', '<div class="logo">S</div><h1 class="title">SCORA</h1><p class="tagline">AI fitness coach that reads your body and adapts your training.</p><ul><li>Daily posture reads</li><li>Real-time adaptation</li><li>Strava + Oura integration</li><li>Human coach marketplace</li></ul><div class="footer"><p><a href="/privacy">Privacy</a> — <a href="/terms">Terms</a></p></div>'));
});

fastify.get('/privacy', async (request, reply) => {
  return reply.type('text/html').send(`<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{background:#000;color:#fff;font-family:system-ui;margin:0;padding:40px}div{max-width:600px;margin:0 auto}h1{font-size:32px;margin-bottom:8px}p{color:#999}p.date{font-size:13px;margin-bottom:30px}p.content{color:#ccc;line-height:1.6}</style></head><body><div><h1>Privacy Policy</h1><p class="date">Updated July 1, 2026</p><p class="content">We collect fitness data from Strava, Oura, and Apple Health with your permission. Your data is protected with industry-standard encryption. Contact hello@scora.app to access or delete your data.</p></div></body></html>`);
});
...
const STRAVA_CLIENT_SECRET = process.env.STRAVA_SECRET || '';
...
fastify.get('/api/auth/strava/callback', async (request, reply) => {
  ...
  if (!code) return reply.type('text/html').send(page('Error', '<h1>Missing Code</h1><p>Authorization failed. Please try again.</p>'));
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      return reply.type('text/html').send(page('Error', '<h1>Auth Failed</h1><p>Could not exchange code. Please try again.</p>'));
    }
    ...
    return reply.type('text/html').send(page('Strava Linked', `<div class="logo">S</div><h1 class="title">Strava Linked</h1><p class="tagline">Welcome, ${athleteName}!</p><a href="${deepLink}" style="display:inline-block;padding:14px 32px;background:#fff;color:#000;border-radius:4px;font-weight:600">Open SCORA</a><div class="footer"><p>If you're not redirected, tap the button above.</p></div>`));
  } catch (error) {
    fastify.log.error(error);
    return reply.type('text/html').send(page('Error', '<h1>Error</h1><p>Failed to link Strava. Please try again.</p>'));
  }
});
...
fastify.get('/api/auth/oura/callback', async (request, reply) => {
  ...
  try {
    ...
    return reply.type('text/html').send(page('Oura Linked', `<div class="logo">S</div><h1 class="title">Oura Linked</h1><p class="tagline">Your sleep data is connected!</p><a href="${deepLink}" style="display:inline-block;padding:14px 32px;background:#fff;color:#000;border-radius:4px;font-weight:600">Open SCORA</a><div class="footer"><p>If you're not redirected, tap the button above.</p></div>`));
  } catch (error) {
