...
fastify.get('/', async (request, reply) => {
  return { message: 'SCORA API', website: 'Coming soon' };
});

fastify.get('/privacy', async (request, reply) => {
  return { message: 'Privacy Policy placeholder' };
});

fastify.get('/terms', async (request, reply) => {
  return { message: 'Terms of Service placeholder' };
});
...
fastify.get('/api/auth/strava/callback', async (request, reply) => {
  ...
  try {
    ...
    return reply.code(500).send({ error: 'Failed to link Strava' });
  }
});

fastify.get('/api/auth/oura', async (request, reply) => {
  return reply.code(501).send({ message: 'Oura OAuth not implemented yet' });
});
