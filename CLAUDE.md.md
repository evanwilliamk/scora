...
Version: v1.4 (2026-04-26) Read this first. Re-read it before any non-trivial change. Every decision in this codebase is downstream of the rules in this file.
...
Free — interpretation layer (voice + dashboard + CDV). Non-prescriptive.
Plan ($14/mo) — Free + adaptive training plan + WhatsApp doorbell + inline chart-expansions + calorie engine + race predictions.
Coach ($200-300/mo via marketplace) — Plan tier + real human coach who reads the athlete's data through SCORA. Coach's plan overrides SCORA's AI plan.
Coach-side platform — free for coaches. Multi-athlete dashboard, workout builder, plan templates, athlete monitoring. SCORA takes 15% of athlete-coach marketplace transactions.
...
Adapted by deterministic rules (see §5.3). Never adapted by LLM vibes.
...
The voice can observe that an adaptation happened, since the plan engine already made the decision: Allowed: "Today's tempo was moved to Wednesday because you slept 5.5 hours Monday. Body and plan will align better."
...
3.3 Numeric-first design principle (Evan, 2026-04-26)
...
3.4 "Actualize and back up the voice" (Evan, 2026-04-26)
...
3.5 Banned phrase list (see §4)
Same list as v1.3, with additions for tier awareness. Post-render check on every voice emission.
...
Same as v1.3. Monorepo with apps/ios, apps/api, packages/voice, packages/validator, packages/posture, packages/plan-templates (new), packages/plan-engine (new), packages/llm-prompts.
...
CLAUDE.md (this file)
02_SCORA_PRD_v1.4.md — product spec
07_SCORA_Architecture_Data_Model_v2.md — system architecture + schemas
08_SCORA_LLM_Prompt_Spec_v2.md — production prompts + validators
06_SCORA_Pattern_Library_v0.3.md — voice patterns + plan-vs-body subfamily
...
The voice never prescribes. The plan does. These are separate objects with separate rules.
Every claim is backed by a driver. Every driver is tappable to a chart. Trust comes from verifiable data.
Coaches are partners in the marketplace, not competitors. Free tier and Plan tier serve the pre-coach segment. Coach tier is graduation.
